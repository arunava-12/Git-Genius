  ...
  return (
    <motion.div ref={ref} className="fixed inset-x-0 top-0 z-50 w-full">
      ...
      >
          ...
          {routes.map((route) => {
            ...
            return (
              ...
              >
                {route.name}
                {isActive && (
                  <span className="absolute left-1/2 -bottom-1.5 -translate-x-1/2 h-1 w-6 rounded-full bg-primary/80 dark:bg-primary/60 shadow" />
                )}
              </Link>
            );
          })}
        ...
        <div
          className={`flex items-center z-10 ${visible ? "gap-1" : "gap-4"}`}
        >
          <ThemeToggle />
          <UserAuthButton />
        </div>
